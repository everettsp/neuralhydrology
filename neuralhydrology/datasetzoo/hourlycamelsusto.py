...
from neuralhydrology.datasetzoo import camelsus, camusto, hourlycamelsus
from neuralhydrology.utils.config import Config

LOGGER = logging.getLogger(__name__)


class HourlyCamelsUSTO(camelsus.CamelsUS):
    ...
    def __init__(self,
        ...
        super(HourlyCamelsUSTO, self).__init__(cfg=cfg,
                                             is_train=is_train,
                                             period=period,
                                             basin=basin,
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        ...
        if basin[:5] == "000HY":
            ...
            basin_camus = basin[3:]
            df = camusto.load_timeseries(data_dir=self.cfg.data_dir.parent / "CAMUS_TO", basin=basin_camus, forcing="camusto_1h")
        
            rename_forcings = {"discharge(mm)": "qobs_mm_per_hour",
                               "precipitation(mm)":"total_precipitation",
                               "stn_press(kPa)": "pressure",
                               "temperature(C)": "temperature"}
            df = df.rename(columns=rename_forcings)
            ...
            with load_hourly_us_netcdf(self.cfg.data_dir.parent / "CAMELS_US", forcings=self.cfg.forcings[0]) as ds:
                df = ds.sel(basin=basin).to_dataframe()
